/*
Author: Arihant Jain
Runs the Group Marching Tree algorithm with Bubble sampling
*/
	...
	float *d_costs, int *d_edges, int initial_idx, int goal_idx)
{
	...
	const int blockSize = 128;
	const int gridSize = std::min((samplesCount + blockSize - 1) / blockSize, 2147483647);
	setupArraysBub<<<gridSize, blockSize>>>(
	...
	// TODO: add the bubble logic
	attachWavefrontBub<<<gridSize,blockSize>>>(
		samplesCount, d_samples, d_radii, d_initial, 
		r, goal_idx, d_wavefront_ptr, d_edges,
		obstaclesCount, d_obstacles, 
		d_costs, d_costGoal, d_debugOutput_ptr);
	cudaDeviceSynchronize();
	code = cudaPeekAtLastError();
	if (cudaSuccess != code) { std::cout << "ERROR on attachWavefrontBub: " << cudaGetErrorString(code) << std::endl; }
	...
	{ 
		...
		fillWavefrontBub<<<gridSize, blockSize>>>(samplesCount,
		...
		findWavefrontBub<<<gridSizeActive, blockSizeActive>>>(
		...
		fillWavefrontBub<<<gridSize, blockSize>>>(
		...
		findOptimalConnectionBub<<<gridSizeActiveExp, blockSizeActiveExp>>>(
		...
		verifyExpansionBub<<<gridSizeActiveVerify, blockSizeActiveVerify>>>(
	...
	float *d_costs, int *d_edges, int initial_idx)
{
	...
	// std::cout << "attaching wavefront to "<< initial[0] << ", " << initial[1] << ", " << initial[2] << ", " << initial[3] << std::endl;
	attachWavefrontBub<<<gridSize,blockSize>>>(
		samplesCount, d_samples, d_radii, d_initial, r, goal_idx, d_wavefront_ptr, d_edges,
...
void GMTBwavefront(float *initial, float *goal, float *d_obstacles, int obstaclesCount,
	float *d_distancesCome, int *d_nnGoEdges, int *d_nnComeEdges, int nnSize, float *d_discMotions, int *d_nnIdxs,
	float *d_samples, float *d_radii, int samplesCount, bool *d_isFreeSamples, float r, int numDisc,
	float *d_costs, int *d_edges, int initial_idx, int goal_idx)
{
	double t_gmtStart = std::clock();
	int maxItrs = 100; 
	...
	const int blockSize = 128;
	const int gridSize = std::min((samplesCount + blockSize - 1) / blockSize, 2147483647);
	setupArraysBub<<<gridSize, blockSize>>>(d_wavefront_ptr, d_wavefrontNew_ptr, 
		d_wavefrontWas, d_unvisited, d_isFreeSamples, d_costGoal, d_costs, d_edges, samplesCount);
	...
	{ 
		...
		verifyExpansionBub<<<gridSizeActiveVerify, blockSizeActiveVerify>>>(
			obstaclesCount, d_wavefrontNew_ptr, d_edges,
			d_samples, d_radii, d_obstacles, d_costs, 
			d_nnIdxs, d_discMotions, numDisc, d_isCollision,
			activeSize, d_wavefrontActiveIdx_ptr, d_debugOutput_ptr);
		removeInvalidExpansions<<<gridSizeActiveExp, blockSizeActiveExp>>>(
			obstaclesCount, d_wavefrontNew_ptr, d_edges,
...
__global__
void findWavefrontBub(bool *unvisited, bool *wavefront, bool *wavefrontNew, bool *wavefrontWas,
...
__global__
void fillWavefrontBub(int samplesCount, int *wavefrontActiveIdx, int *wavefrontScanIdx, bool* wavefront)
{
	int node = blockIdx.x * blockDim.x + threadIdx.x;
	if (node >= samplesCount)
		return;
...
__global__
void findOptimalConnectionBub(bool *wavefront, int* edges, float* costs, float *distancesCome, 
...
__global__
void verifyExpansionBub(int obstaclesCount, bool *wavefrontNew, int* edges,
	float *samples, float *radii, float* obstacles, float* costs,
	...
	int wavefrontSize, int* wavefrontActiveIdx, float* debugOutput)
{
	...
	// TODO: figure out the radii stuff
...
__global__
void attachWavefrontBub(int samplesCount, float *samples, float *initial, 
